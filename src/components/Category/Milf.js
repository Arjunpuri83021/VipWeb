...
function Milf() {
    ...
    const [search, setSearch] = useState("milf");
    ...
    useEffect(() => {
            if (currentPage === 1 && window.location.pathname !== '/category/milfnut') {
                navigate('/category/milfnut');  // Redirect to root URL
            }
    ...
    useEffect(() => {
        ...
        const canonicalUrl = `https://vipmilfnut.com/category/milfnut/${currentPage === 1 ? '' : currentPage}`;
    ...
    const fetchData = async (page = 1, searchQuery = "milf") => {
        setLoading(true);
        setError(null);
        try {
    ...
    const handlePageChange = (event, value) => {
        navigate(`/category/milfnut/${value}`); // Update URL with new page
        window.scrollTo(0, 0); // Scroll to top after page change
    };

    const handleSearch = (query) => {
        setSearch(query || "milf");
        navigate(`/category/milfnut/1`); // Reset to page 1 on search
    };
