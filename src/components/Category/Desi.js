...
function Desi() {
    ...
    const [search, setSearch] = useState("desi");
    ...
    useEffect(() => {
            if (currentPage === 1 && window.location.pathname !== '/category/desi49') {
                navigate('/category/desi49');  // Redirect to root URL
            }
        }, [currentPage, navigate]);

    useEffect(() => {
        document.title = `VipMilfNut desi49 sex Videos page ${currentPage} – Watch hot desi clips in HD`;
    ...
    const fetchData = async (page = 1, searchQuery = "desi") => {
        setLoading(true);
        setError(null);
        try {
    ...
    const handlePageChange = (event, value) => {
        navigate(`/category/desi49/${value}`); // Update URL with new page
        window.scrollTo(0, 0); // Scroll to top after page change
    };

    const handleSearch = (query) => {
        setSearch(query || "desi");
        navigate(`/category/desi49/1`); // Reset to page 1 on search
    };
    ...
    return (
        <>
            <Helmet>
                <title>VipMilfNut Desi49 Sex Videos – Watch hot desi clips in HD</title>
            ...
            <div style={{ width: "95%", margin: "auto" }}>
                <h1 style={{fontSize:"18px", textAlign:"center", marginTop:"10px"}}>VipMilfNut Desi sex Videos</h1>
                <SmartLinkBanner />
                {loading && <p>Loading...</p>}
