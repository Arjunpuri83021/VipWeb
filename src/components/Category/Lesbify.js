...
function Lesbify() {
    ...
    const [search, setSearch] = useState("lesbian");
    ...
    useEffect(() => {
        if (currentPage === 1 && window.location.pathname !== '/category/lesbify') {
            navigate('/category/lesbify');  // Redirect to root URL
        }
    }, [currentPage, navigate]);

    useEffect(() => {
        document.title = `VipMilfNut Lesbify Videos page ${currentPage} - xxxhd,wwwsexcom videos`;
    ...
    const fetchData = async (page = 1, searchQuery = "lesbian") => {
        setLoading(true);
        setError(null);
        try {
    ...
    const handlePageChange = (event, value) => {
        navigate(`/category/lesbify/${value}`); // Update URL with new page
        window.scrollTo(0, 0); // Scroll to top after page change
    };

    const handleSearch = (query) => {
        setSearch(query || "lesbian");
        navigate(`/category/lesbify/1`); // Reset to page 1 on search
    };
    ...
    return (
        <>
            <Helmet>
                <title>VipMilfNut Lesbify Videos - xxxhd,wwwsexcom videos</title>
            ...
            <div style={{ width: "95%", margin: "auto" }}>
                <h1 style={{fontSize:"18px", textAlign:"center", marginTop:"10px"}}>VipMilfNut Lesbify - Lesbian Videos</h1>
                <SmartLinkBanner />
                {loading && <p>Loading...</p>}
