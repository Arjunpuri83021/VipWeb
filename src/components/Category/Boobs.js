...
function Boobs() {
    ...
    const [search, setSearch] = useState("boobs");
    ...
    useEffect(() => {
            if (currentPage === 1 && window.location.pathname !== '/category/boobs-pressing') {
                navigate('/category/boobs-pressing');  // Redirect to root URL
            }
    ...
    const fetchData = async (page = 1, searchQuery = "Boobs") => {
        setLoading(true);
        setError(null);
        try {
    ...
    const handlePageChange = (event, value) => {
        navigate(`/category/boobs-pressing/${value}`); // Update URL with new page
        window.scrollTo(0, 0); // Scroll to top after page change
    };

    const handleSearch = (query) => {
        setSearch(query || "Boobs");
        navigate(`/category/boobs-pressing/1`); // Reset to page 1 on search
    };
