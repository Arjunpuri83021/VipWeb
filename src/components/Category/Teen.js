...
function Teen() {
    ...
    useEffect(() => {
            if (currentPage === 1 && window.location.pathname !== '/category/teen-sex') {
                navigate('/category/teen-sex');  // Redirect to root URL
            }
        }, [currentPage, navigate]);

    useEffect(() => {
        document.title = `VipMilfNut teen sex Videos page ${currentPage} - scout69 pornmz xvedeo`;
    ...
    const fetchData = async (page = 1, searchQuery = "teen") => {
        setLoading(true);
        setError(null);
        try {
    ...
    const handlePageChange = (event, value) => {
        navigate(`/category/teen-sex/${value}`); // Update URL with new page
        window.scrollTo(0, 0); // Scroll to top after page change
    };

    const handleSearch = (query) => {
        setSearch(query || "teen");
        navigate(`/category/teen-sex/1`); // Reset to page 1 on search
    };
    ...
    return (
        <>
            <Helmet>
                <title>VipMilfNut teen sex Videos - scout69 pornmz xvedeo</title>
            ...
            <div style={{ width: "95%", margin: "auto" }}>
                <h1 style={{fontSize:"18px", textAlign:"center", marginTop:"10px"}}>VipMilfNut teen sex - teen Videos</h1>
